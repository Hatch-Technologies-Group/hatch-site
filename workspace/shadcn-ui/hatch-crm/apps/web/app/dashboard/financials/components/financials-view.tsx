...
export function FinancialsView({ orgId }: { orgId: string }) {
  ...
  return (
    <Card className="space-y-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
      ...
      <div className="space-y-1 text-sm text-slate-600">
        ...
        <p>
          Realm ID:{' '}
          <span className="font-mono text-slate-900">{config?.realmId ?? '—'}</span>
        </p>
        <p className="text-xs text-slate-500">
          Connected at:{' '}
          {config?.connectedAt ? new Date(config.connectedAt).toLocaleString() : 'Not yet connected'}
        </p>
      </div>
            ...
            {isLoading ? (
              ...
              records.map((record) => {
                ...
                return (
                  ...
                  <td className="px-4 py-3">
                    <Badge className={cn('bg-slate-100 text-slate-700', record.syncStatus === 'FAILED' && 'bg-rose-50 text-rose-600')}>
                      {record.syncStatus}
                    </Badge>
                  </td>
