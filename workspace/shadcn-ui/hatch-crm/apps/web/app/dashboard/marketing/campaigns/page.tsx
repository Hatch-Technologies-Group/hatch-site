...
import { listMarketingCampaigns } from '@/lib/api/marketing';
import type {
  CampaignFilter,
  MarketingCampaign,
...
function channelBadge(channel: MarketingChannel) {
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            AI-powered emails and SMS campaigns. Draft with Haven or Lumen, review, and schedule.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" size="sm" asChild>
            <Link href="/dashboard/marketing/studio">Marketing Studio</Link>
          </Button>
                  ...
                  <table className="w-full border-collapse text-xs">
                    ...
                    <tbody>
                      {filteredCampaigns.map((campaign) => (
                        <tr key={campaign.id} className={cn('border-b last:border-0 hover:bg-muted/40')}>
                          ...
                          <td className="px-3 py-2 text-left">{formatDateTime(campaign.sentAt ?? null)}</td>
