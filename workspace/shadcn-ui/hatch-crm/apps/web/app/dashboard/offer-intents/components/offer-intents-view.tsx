...
export function OfferIntentsView({ orgId }: OfferIntentsViewProps) {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () =>
      fetchOfferIntents(orgId, statusFilter === 'ALL' ? undefined : { status: statusFilter }),
    staleTime: 30_000
  });
  ...
  return (
    <tr className="border-t border-slate-100">
      ...
      <td className="px-4 py-3">
        ...
        {offer.contingencies ? (
          <p className="text-xs text-slate-500">Contingencies: {offer.contingencies}</p>
        ) : null}
