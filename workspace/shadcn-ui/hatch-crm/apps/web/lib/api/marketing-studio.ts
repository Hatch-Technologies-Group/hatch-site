...
export async function listMarketingStudioAssets(orgId: string, listingId: string): Promise<{
  assets: MarketingStudioAsset[];
}> {
  return apiFetch(`organizations/${orgId}/marketing-studio/listings/${listingId}/assets`);
}

export async function listMarketingStudioListingImages(orgId: string, listingId: string): Promise<{
