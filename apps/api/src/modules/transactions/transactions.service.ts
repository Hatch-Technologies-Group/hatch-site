...
export class TransactionsService {
  ...
  async createTransaction(orgId: string, creatorUserId: string, dto: CreateTransactionDto) {
    ...
    const transaction = await this.prisma.orgTransaction.create({
      data: {
  ...
  async updateTransaction(orgId: string, userId: string, transactionId: string, dto: UpdateTransactionDto) {
    ...
    const updateData = {
      status: dto.status ? (dto.status as OrgTransactionStatus) : undefined,
      buyerName: dto.buyerName ?? undefined,
      sellerName: dto.sellerName ?? undefined,
    ...
    if (!isBroker) {
      delete updateData.complianceNotes;
      delete updateData.isCompliant;
      delete updateData.requiresAction;
      delete updateData.status;
    }
    return this.prisma.orgTransaction.update({ where: { id: transactionId }, data: updateData });
  }
