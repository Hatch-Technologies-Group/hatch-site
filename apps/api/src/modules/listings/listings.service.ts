...
export class ListingsService {
  ...
  async createListing(orgId: string, creatorUserId: string, dto: CreateListingDto) {
    ...
    if (dto.agentProfileId) {
      const profile = await this.assertAgentProfile(dto.agentProfileId, orgId);
      agentProfileId = profile.id;
    }

    const listing = await this.prisma.orgListing.create({
      data: {
  ...
  async updateListing(orgId: string, userId: string, listingId: string, dto: UpdateListingDto) {
    ...
    if (!isBroker) {
      if (dto.status !== undefined || dto.agentProfileId !== undefined) {
        throw new ForbiddenException('Agents cannot modify status or agent assignments');
      }
    }

    let agentProfileId = listing.agentProfileId;
    if (dto.agentProfileId !== undefined) {
      agentProfileId = dto.agentProfileId === null ? null : (await this.assertAgentProfile(dto.agentProfileId, orgId)).id;
    }
