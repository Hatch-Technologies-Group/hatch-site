...
describeIf(RUN_INTEGRATION)('Phase 8 — Listings backbone', () => {
  ...
  it('allows agent to create listing, request approval, broker approve, attach documents, and surface stats', async () => {
    const orgId = await createOrg(`Listings Org ${Date.now()}`);
    const agentEmail = `listing_agent_${Date.now()}@example.com`;
    const acceptRes = await acceptInvite(orgId, agentEmail);
    const prisma = (app as any).prisma as import('@hatch/db').PrismaClient;
    ...
    const approveRes = await withBroker(orgId, request(app.getHttpServer()).post(`/organizations/${orgId}/listings/${listingId}/approve`)).expect(200);
    expect(approveRes.body.status).toBe('ACTIVE');
    expect(approveRes.body.brokerApproved).toBe(true);

    const orgFile = await createOrgFile(orgId, agentUser.id, 'listing-doc');
    await withBroker(orgId, request(app.getHttpServer()).post(`/organizations/${orgId}/listings/${listingId}/documents`))
  ...
  it('prevents non-members from modifying listings', async () => {
    ...
    const listing = await withBroker(orgId, request(app.getHttpServer()).post(`/organizations/${orgId}/listings`))
      .send({ addressLine1: '456 Elm', city: 'Tampa', state: 'FL', postalCode: '33602' })
      .expect(201);
    const outsiderToken = `Bearer ${jwt.sign({ sub: 'random-user', tenantId, orgId: 'other-org', roles: ['agent'], role: 'AGENT' }, process.env.JWT_ACCESS_SECRET ?? process.env.API_JWT_SECRET ?? 'dev-secret', { expiresIn: '1h' })}`;
    await request(app.getHttpServer())
